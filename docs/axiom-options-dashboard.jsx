...
const BS = {
  ...
  normPDF(x) { return 0.3989422820 * Math.exp(-x * x / 2); },
  d1(S, K, T, r, q, sigma) { return (Math.log(S/K) + (r - q + 0.5*sigma*sigma)*T) / (sigma*Math.sqrt(T)); },
  d2(d1, sigma, T) { return d1 - sigma * Math.sqrt(T); },
  ...
  ivFromPrice(price, S, K, T, r, q, isCall) {
    let lo = 0.001, hi = 5.0, iv = 0.2;
    for (let i = 0; i < 50; i++) {
      const mid = (lo + hi) / 2;
      const p = isCall ? this.callPrice(S,K,T,r,q,mid) : this.putPrice(S,K,T,r,q,mid);
      if (Math.abs(p - price) < 0.001) return mid;
      p > price ? hi = mid : lo = mid;
    }
    return (lo + hi) / 2;
  }
};
...
// ─── DB: real-ish stock data ──────────────────────────────────
const STOCKS = {
  AAPL: { price:211.50, hv20:22.4, hv60:24.1, ivRank:42, beta:1.24, pe:33.2, earnings:"Jan 30" },
  NVDA: { price:875.40, hv20:48.2, hv60:52.1, ivRank:38, beta:1.68, pe:68.4, earnings:"Feb 26" },
  TSLA: { price:175.20, hv20:58.4, hv60:62.3, ivRank:55, beta:2.31, pe:48.2, earnings:"Jan 29" },
  MSFT: { price:415.20, hv20:18.2, hv60:20.4, ivRank:28, beta:0.91, pe:35.8, earnings:"Jan 29" },
  META: { price:512.00, hv20:32.4, hv60:35.2, ivRank:45, beta:1.22, pe:27.1, earnings:"Jan 29" },
  AMZN: { price:189.50, hv20:28.4, hv60:30.2, ivRank:35, beta:1.15, pe:41.2, earnings:"Feb 6"  },
  SPY:  { price:524.80, hv20:14.2, hv60:16.1, ivRank:32, beta:1.00, pe:22.1, earnings:"N/A"    },
  QQQ:  { price:448.20, hv20:18.4, hv60:20.2, ivRank:36, beta:1.10, pe:28.4, earnings:"N/A"    },
  GOOGL:{ price:172.80, hv20:24.2, hv60:26.4, ivRank:30, beta:1.06, pe:22.4, earnings:"Feb 4"  },
  AMD:  { price:164.80, hv20:52.4, hv60:56.2, ivRank:62, beta:1.72, pe:214,  earnings:"Jan 28" },
};

// ─── Options Strategy Selector ────────────────────────────────
function selectStrategy(params) {
  const { ivRank, ivProxy, vrp, trend, adx, pcr, gexPositive, hasEarnings } = params;
  const strategies = [
    {
      ...
      when:"Bullish + IV cheap + strong trend",
      setup:"Buy ATM or slightly OTM call",
      maxRisk:"Premium paid", maxProfit:"Unlimited",
      bestIV:"Low (<30%ile)", bestFor:"Earnings run-up, breakout"
    },
    {
      ...
      when:"Bearish + IV cheap + strong downtrend",
      setup:"Buy ATM or slightly OTM put",
      maxRisk:"Premium paid", maxProfit:"Near unlimited",
      bestIV:"Low (<30%ile)", bestFor:"Earnings miss, breakdown"
    },
    {
      ...
      when:"Own stock + IV elevated + neutral-bullish",
      setup:"Own 100 shares, sell OTM call",
      maxRisk:"Stock ownership", maxProfit:"Strike - cost + premium",
      bestIV:"High (>50%ile)", bestFor:"Income generation, plateau stocks"
    },
    {
      ...
      when:"Want to own stock at lower price + IV high",
      setup:"Sell OTM put, hold cash for assignment",
      maxRisk:"Strike - premium", maxProfit:"Premium collected",
      bestIV:"High (>50%ile)", bestFor:"Accumulation, income"
    },
    {
      ...
      when:"Range-bound + IV high + low ADX",
      setup:"Sell OTM call spread + sell OTM put spread",
      maxRisk:"Width of spread - premium", maxProfit:"Premium collected",
      bestIV:"High (>50%ile)", bestFor:"Sideways market, post-earnings"
    },
    {
      ...
      when:"Big move expected + IV cheap + catalyst",
      setup:"Buy ATM call + ATM put (straddle)",
      maxRisk:"Total premium paid", maxProfit:"Unlimited",
      bestIV:"Very low (<25%ile)", bestFor:"Earnings, FDA, major events"
    },
    {
      id:"bull_spread", name:"Bull Call Spread", emoji:"📊", risk:"Defined",
      score: (trend==="bull"?30:0) + (ivRank>35?20:0) + (pcr<0.7?15:0) + (adx>20?15:0),
      when:"Bullish + IV elevated (reduce cost)",
      setup:"Buy ATM call, sell OTM call at target",
      maxRisk:"Net debit", maxProfit:"Width - debit",
      bestIV:"Medium (30-60%ile)", bestFor:"Moderate bull moves"
    },
    {
      id:"bear_spread", name:"Bear Put Spread", emoji:"📉", risk:"Defined",
      score: (trend==="bear"?30:0) + (ivRank>35?20:0) + (pcr>1.2?15:0) + (adx>20?15:0),
      when:"Bearish + IV elevated (reduce cost)",
      setup:"Buy ATM put, sell OTM put at target",
      maxRisk:"Net debit", maxProfit:"Width - debit",
      bestIV:"Medium (30-60%ile)", bestFor:"Moderate bear moves"
    },
    {
      id:"calendar", name:"Calendar Spread", emoji:"📅", risk:"Defined",
      score: (ivRank<30?30:0) + (trend==="neutral"?25:0) + (!hasEarnings?15:0) + (vrp<-2?10:0),
      when:"Low IV now, expect IV rise, range-bound",
      setup:"Sell near-term option, buy far-term same strike",
      maxRisk:"Net debit", maxProfit:"Near-term decays faster",
      bestIV:"Low (<30%ile)", bestFor:"Theta play, volatility expansion"
    },
    {
      ...
      when:"Strong directional bias + very high IV",
      setup:"Buy 1 ATM, sell 2 OTM (same expiry)",
      maxRisk:"Unlimited beyond short strikes", maxProfit:"Net credit + move to short",
      bestIV:"Very high (>60%ile)", bestFor:"High IV environment with direction"
    },
  ];
  return strategies.map(s => ({ ...s, score: Math.min(100, Math.max(0, s.score)) }))
    .sort((a,b) => b.score - a.score);
}

// ─── P&L Chart at expiration ─────────────────────────────────
function PLChart({ strategy, S, K, T, r, q, sigma, premium, canvas }) {
  ...
  useEffect(() => {
    ...
    const pnl = prices.map(p => {
      switch(strategy) {
        ...
        case "iron_condor": {
          const w = K * 0.05;
          const callSpread = Math.min(0, Math.max(-w, K + w*0.5 - p));
          const putSpread  = Math.min(0, Math.max(-w, p - (K - w*0.5)));
          return (premium - Math.abs(callSpread) - Math.abs(putSpread)) * 100;
        }
        case "straddle": return (Math.abs(p - K) - premium) * 100;
        case "bull_spread": {
          const w = K * 0.05;
          return (Math.min(w, Math.max(0, p - K)) - premium) * 100;
        }
        case "bear_spread": {
          const w = K * 0.05;
          return (Math.min(w, Math.max(0, K - p)) - premium) * 100;
        }
        case "covered_call": return (Math.min(K, p) - S + premium) * 100;
        case "cash_secured_put": return (Math.max(0, p - (K - premium)) - (K - (K - premium))) * 100;
    ...
    const toX = i => padL + (i / (prices.length-1)) * cW;
    const toY = v => padT + (1 - (v - minPnl) / range) * cH;
    ...
    // Fill
    ctx.beginPath();
    prices.forEach((p, i) => i===0 ? ctx.moveTo(toX(i), toY(pnl[i])) : ctx.lineTo(toX(i), toY(pnl[i])));
    ...
    ctx.beginPath();
    prices.forEach((p, i) => {
      i===0 ? ctx.moveTo(toX(i), toY(pnl[i])) : ctx.lineTo(toX(i), toY(pnl[i]));
    });
    ...
    const curX = toX(prices.findIndex(p => p >= S));
    ...
    [0, 15, 30, 45, 59].forEach(i => {
      ctx.fillText("$"+Math.round(prices[i]), toX(i), H-6);
    });
...
// ─── Greeks display ───────────────────────────────────────────
function GreeksCard({ greeks, isCall }) {
  const items = [
...
export default function App() {
  ...
  const [analysisTab, setAnalysisTab] = useState("strategy");
  ...
  // IV with small random variation per ticker
  const ivProxy = useMemo(() => {
    const rng2 = seededRng(strSeed(ticker + "iv"));
    return stock.hv20 * (1.10 + rng2() * 0.25);
  }, [ticker, stock]);
  ...
  const K  = customStrike ? parseFloat(customStrike) : Math.round(S / 5) * 5;
  ...
  // VRP
  const vrp = ivProxy - stock.hv20;
  const ivRank = stock.ivRank;
  const pcr = ivRank > 50 ? 1.3 - (ivRank-50)*0.01 : 0.7 + (50-ivRank)*0.01;
  const trend = stock.hv20 > stock.hv60 ? "bear" : stock.ivRank < 35 ? "bull" : "neutral";
  const hasEarnings = stock.earnings !== "N/A";

  const strategies = useMemo(() => selectStrategy({
    ivRank, ivProxy, vrp, trend,
    adx: 20 + rng() * 15,
    pcr, gexPositive: ivProxy < stock.hv60, hasEarnings
  }), [ticker, dte]);

  const best = strategies[0];
  const activeStrat = selStrat || best?.id;

  // Premium for selected strategy
  const stratPremium = {
  ...
  const STRAT_COLOR = {
    ...
    "ratio_spread":"#e879f9","diagonal":"#34d399"
  };
  ...
  return (
    <div style={{ minHeight:"100vh", background:"#06101a", fontFamily:"JetBrains Mono,Fira Code,monospace", color:"#c8d6e0" }}>
      ...
      <div style={{ position:"relative", zIndex:1, maxWidth:1080, margin:"0 auto", padding:"18px 12px 48px" }}>
        ...
        <div style={{ textAlign:"center", marginBottom:18 }}>
          <div style={{ display:"inline-flex", alignItems:"center", gap:8, marginBottom:8 }}>
            ...
            <span style={{ fontSize:9, letterSpacing:4, color:"#00ff88" }}>AXIOM OPTIONS STRATEGY ENGINE v1.0</span>
          ...
          <p style={{ margin:0, fontSize:10, color:"#1e3040", letterSpacing:2 }}>BLACK-SCHOLES · GREEKS · IV RANK · GEX · MAX PAIN · STRATEGY SELECTOR · P&L CHARTS</p>
        </div>
        ...
        <div style={{ background:"rgba(255,255,255,0.022)", border:"1px solid rgba(0,255,136,0.1)", borderRadius:14, padding:16, marginBottom:14 }}>
          <div style={{ display:"flex", gap:10, flexWrap:"wrap", alignItems:"flex-end" }}>
            <div>
              ...
              <select value={ticker} onChange={e=>setTicker(e.target.value)} style={{ background:"rgba(0,0,0,0.5)", border:"1px solid rgba(0,255,136,0.3)", borderRadius:7, padding:"8px 12px", color:"#00ff88", fontSize:16, fontWeight:800, fontFamily:"inherit", letterSpacing:2, outline:"none" }}>
                {Object.keys(STOCKS).map(s=><option key={s} value={s}>{s}</option>)}
              </select>
            </div>
            <div>
              <div style={{ fontSize:8, letterSpacing:2, color:"#2a4050", marginBottom:5 }}>DAYS TO EXPIRY</div>
              <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
                {[7,14,21,30,45,60,90].map(d=>(
                  <button key={d} onClick={()=>setDte(d)} style={{ padding:"7px 10px", borderRadius:6, fontSize:10, fontFamily:"inherit", fontWeight:600, cursor:"pointer", background:dte===d?"rgba(0,255,136,0.12)":"rgba(255,255,255,0.03)", border:dte===d?"1px solid rgba(0,255,136,0.55)":"1px solid rgba(255,255,255,0.06)", color:dte===d?"#00ff88":"#3d5a6e" }}>
                    {d}d
                  </button>
                ))}
              </div>
            ...
            <div>
              <div style={{ fontSize:8, letterSpacing:2, color:"#2a4050", marginBottom:5 }}>CUSTOM STRIKE</div>
              <input value={customStrike} onChange={e=>setCustomStrike(e.target.value)} placeholder={`$${Math.round(S/5)*5}`} style={{ width:90, background:"rgba(0,0,0,0.4)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:7, padding:"8px 10px", color:"#c8d6e0", fontSize:13, fontFamily:"inherit", outline:"none" }}/>
            </div>
          </div>

          {/* Stock snapshot */}
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(80px,1fr))", gap:6, marginTop:12 }}>
            {[
              { l:"PRICE", v:`$${S}`, c:"#c8d6e0" },
              { l:"IV (PROXY)", v:`${ivProxy.toFixed(1)}%`, c:ivProxy>stock.hv60?"#ef4444":"#00ff88" },
              { l:"HV20", v:`${stock.hv20}%`, c:"#94a3b8" },
              { l:"VRP", v:`${vrp>0?"+":""}${vrp.toFixed(1)}%`, c:vrp>0?"#ef4444":"#00ff88" },
              { l:"BETA", v:stock.beta, c:"#94a3b8" },
              { l:"EARNINGS", v:stock.earnings, c:"#a78bfa" },
              ...
              <div key={item.l} style={{ background:"rgba(255,255,255,0.018)", border:"1px solid rgba(255,255,255,0.05)", borderRadius:6, padding:"6px 8px", textAlign:"center" }}>
                ...
                <div style={{ fontSize:11, fontWeight:700, color:item.c, fontFamily:"monospace" }}>{item.v}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
          ...
          <div>
            ...
            <div style={{ background:"rgba(255,255,255,0.018)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:10, padding:14, marginBottom:10 }}>
              ...
              <div style={{ display:"flex", gap:14, alignItems:"center" }}>
                ...
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:10, color:"#4a6070", lineHeight:1.7 }}>
                    ...
                    <div>IV%ile: <strong style={{ color:"#c8d6e0" }}>{Math.round(ivRank*0.9)}%</strong></div>
                    <div>VRP: <strong style={{ color:vrp>0?"#ef4444":"#00ff88" }}>{vrp>0?"+":""}{vrp.toFixed(1)}%</strong></div>
                    <div>Term: <strong style={{ color:stock.hv20<stock.hv60?"#00ff88":"#ef4444" }}>{stock.hv20<stock.hv60?"CONTANGO":"BACKWARDATION"}</strong></div>
                    <div>GEX: <strong style={{ color:ivProxy<stock.hv60?"#00ff88":"#ef4444" }}>{ivProxy<stock.hv60?"POSITIVE (Range)":"NEGATIVE (Trending)"}</strong></div>
                  </div>
                </div>
              </div>
              {/* Expected move bands */}
              <div style={{ marginTop:10, padding:"8px 10px", background:"rgba(0,212,255,0.06)", border:"1px solid rgba(0,212,255,0.2)", borderRadius:6 }}>
                <div style={{ fontSize:8, color:"#2a4050", marginBottom:4 }}>EXPECTED MOVE ({dte}d) — 1 STANDARD DEVIATION</div>
                <div style={{ display:"flex", justifyContent:"space-between", fontSize:11, fontFamily:"monospace" }}>
                  ...
                  <span style={{ color:"#c8d6e0" }}>${S} ±{expMovePct.toFixed(1)}%</span>
            ...
            <div style={{ background:"rgba(255,255,255,0.018)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:10, padding:14 }}>
              ...
              {strategies.map((s, i) => {
                ...
                const sc = STRAT_COLOR[s.id] || "#94a3b8";
                return (
                  <div key={s.id} onClick={()=>setSelStrat(s.id)}
                    style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"7px 8px", marginBottom:4, borderRadius:6, cursor:"pointer", background:active?`${sc}12`:"rgba(255,255,255,0.01)", border:`1px solid ${active?sc+"40":"rgba(255,255,255,0.04)"}` }}>
                    <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                      <span style={{ fontSize:14 }}>{s.emoji}</span>
                      <div>
                        <div style={{ fontSize:10, fontWeight:700, color:active?sc:"#c8d6e0" }}>{s.name}</div>
                    ...
                    <div style={{ textAlign:"right" }}>
                      <div style={{ fontSize:11, fontWeight:700, color:s.score>=70?sc:"#94a3b8", fontFamily:"monospace" }}>{s.score}</div>
                      <div style={{ width:40, height:3, background:"rgba(255,255,255,0.06)", borderRadius:2, overflow:"hidden" }}>
                        <div style={{ height:"100%", width:s.score+"%", background:s.score>=70?sc:"#3d5a6e", borderRadius:2 }}/>
                      </div>
                    </div>
                  </div>
                );
          ...
          {/* RIGHT: Strategy detail + Greeks + PnL */}
          <div>
            {(() => {
              ...
              return (
                <>
                  ...
                  <div style={{ display:"flex", gap:5, marginBottom:10 }}>
                    {[["strategy","Strategy"],["greeks","Greeks"],["pnl","P&L Chart"],["chain","Option Chain"]].map(([id,label])=>(
                      <button key={id} onClick={()=>setAnalysisTab(id)} style={{ padding:"5px 10px", borderRadius:5, fontSize:9, fontFamily:"inherit", fontWeight:600, cursor:"pointer", letterSpacing:1, background:analysisTab===id?"rgba(0,255,136,0.12)":"rgba(255,255,255,0.02)", border:analysisTab===id?"1px solid rgba(0,255,136,0.45)":"1px solid rgba(255,255,255,0.06)", color:analysisTab===id?"#00ff88":"#3d5a6e" }}>
                        {label}
                      </button>
                    ))}
                  </div>
                  ...
                  {analysisTab==="pnl" && (
                    <div style={{ background:"rgba(255,255,255,0.018)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:10, padding:14 }}>
                      ...
                      <div style={{ display:"flex", gap:10, marginTop:8, fontSize:9, color:"#3d5a6e", justifyContent:"center" }}>
                        <span style={{ color:"#00ff88" }}>████ Profit zone</span>
                        <span style={{ color:"#ef4444" }}>████ Loss zone</span>
                        <span style={{ color:"rgba(255,255,255,0.4)" }}>│ Current price</span>
                      </div>
                    </div>
                  )}
                  ...
                  {analysisTab==="chain" && (
                    <div style={{ background:"rgba(255,255,255,0.018)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:10, padding:14, overflowX:"auto" }}>
                      <div style={{ fontSize:9, letterSpacing:2, color:"#2a4050", marginBottom:8 }}>OPTIONS CHAIN — {dte}DTE CONTRACTS</div>
                      <table style={{ width:"100%", borderCollapse:"collapse", fontSize:9 }}>
                        ...
                        <tbody>
                          {[-8,-6,-4,-2,0,2,4,6,8].map((offset, i) => {
                            ...
                            const callG = BS.greeks(S, strike, T, r, q, sigma, true);
                            const putG  = BS.greeks(S, strike, T, r, q, sigma, false);
                            const callP = BS.callPrice(S, strike, T, r, q, sigma);
                            const putP  = BS.putPrice(S, strike, T, r, q, sigma);
                            return (
                              <tr key={strike} style={{ background:isATM?"rgba(0,255,136,0.05)":i%2===0?"rgba(255,255,255,0.006)":"transparent", borderBottom:"1px solid rgba(255,255,255,0.03)" }}>
                                <td style={{ padding:"4px 6px", color:"#00ff88", fontFamily:"monospace", textAlign:"center" }}>${callP.toFixed(2)}</td>
                                <td style={{ padding:"4px 6px", color:"#86efac", fontFamily:"monospace", textAlign:"center" }}>{(callG.delta*100).toFixed(0)}%</td>
                                <td style={{ padding:"4px 6px", color:"#fb923c", fontFamily:"monospace", textAlign:"center" }}>{callG.theta.toFixed(3)}</td>
                                <td style={{ padding:"4px 6px", color:"#a78bfa", fontFamily:"monospace", textAlign:"center" }}>{callG.vega.toFixed(3)}</td>
                                <td style={{ padding:"4px 6px", color:isATM?"#fbbf24":"#c8d6e0", fontFamily:"monospace", textAlign:"center", fontWeight:isATM?700:400 }}>${strike}{isATM?" ★":""}</td>
                                <td style={{ padding:"4px 6px", color:"#a78bfa", fontFamily:"monospace", textAlign:"center" }}>{putG.vega.toFixed(3)}</td>
                                <td style={{ padding:"4px 6px", color:"#fb923c", fontFamily:"monospace", textAlign:"center" }}>{putG.theta.toFixed(3)}</td>
                                <td style={{ padding:"4px 6px", color:"#fca5a5", fontFamily:"monospace", textAlign:"center" }}>{(Math.abs(putG.delta)*100).toFixed(0)}%</td>
                                <td style={{ padding:"4px 6px", color:"#ef4444", fontFamily:"monospace", textAlign:"center" }}>${putP.toFixed(2)}</td>
                              </tr>
                            );
                          })}
                        </tbody>
                      </table>
                      <div style={{ marginTop:6, fontSize:8, color:"#2a4050", textAlign:"center" }}>★ = ATM Strike | IV = {ivProxy.toFixed(1)}%</div>
                    </div>
                  )}
                  ...
                  {analysisTab==="strategy" && (
                    <div style={{ background:"rgba(255,255,255,0.018)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:10, padding:14 }}>
                      <div style={{ fontSize:9, letterSpacing:2, color:"#2a4050", marginBottom:10 }}>STRATEGY ANALYSIS & CHECKLIST</div>
                      ...
                      {[
                        ...
                        { label:"Trend alignment", ok: strat?.id==="long_call"?trend==="bull":strat?.id==="long_put"?trend==="bear":true, desc:"Check directional bias" },
                        { label:"VRP edge",          ok: vrp > 2 ? (strat?.id?.includes("condor")||strat?.id?.includes("covered")) : vrp < 0, desc:`VRP: ${vrp.toFixed(1)}%` },
                        { label:"Earnings timing",   ok: strat?.id==="straddle" ? hasEarnings : !hasEarnings, desc:stock.earnings!=="N/A"?`Earnings: ${stock.earnings}`:"No near-term earnings" },
                        { label:"Premium value",     ok: stratPremium > 0.5, desc:`$${(stratPremium*100).toFixed(0)}/contract` },
                        { label:"GEX regime",        ok: strat?.id?.includes("condor") ? ivProxy<stock.hv60 : true, desc:ivProxy<stock.hv60?"Positive (range)":"Negative (trending)" },
                      ].map(item=>(
                      ...
                      <div style={{ marginTop:10, padding:"10px", background:"rgba(0,0,0,0.3)", borderRadius:6, fontFamily:"monospace", fontSize:11, lineHeight:1.9, color:"#8ba0b0" }}>
                        <div><span style={{ color:"#fbbf24" }}>TICKER:</span> <strong style={{ color:"#e2e8f0" }}>{ticker}</strong> @ ${S}</div>
                        ...
                        <div><span style={{ color:"#fbbf24" }}>STRIKE:</span> <strong style={{ color:"#e2e8f0" }}>${K}</strong> ({strat?.id?.includes("condor")?"sell OTM call + put":"ATM"})</div>
                        ...
                        <div><span style={{ color:"#fbbf24" }}>BREAKEVEN:</span> <strong style={{ color:"#e2e8f0" }}>${(strat?.id?.includes("call")||strat?.id?.includes("bull"))?+(K+stratPremium).toFixed(2):+(K-stratPremium).toFixed(2)}</strong></div>
                      </div>
                      ...
                      <div style={{ marginTop:8, padding:"6px 8px", background:"rgba(239,68,68,0.06)", border:"1px solid rgba(239,68,68,0.2)", borderRadius:5, fontSize:9, color:"#94a3b8" }}>
                        ⚠️ Options involve substantial risk. No strategy guarantees profit. Always use defined risk and size positions appropriately. NOT FINANCIAL ADVICE.
                      </div>
                    </div>
                  )}
                </>
        ...
        <div style={{ marginTop:16, background:"rgba(255,255,255,0.012)", border:"1px solid rgba(255,255,255,0.05)", borderRadius:10, padding:14 }}>
          <div style={{ fontSize:9, letterSpacing:2, color:"#2a4050", marginBottom:8 }}>BEST FREE TOOLS TO USE ALONGSIDE THIS ENGINE</div>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:6, fontSize:10, color:"#4a6070" }}>
            {[
              { name:"FlashAlpha", url:"flashalpha.com", desc:"Free GEX, IV rank, dealer positioning, max pain for 6000+ stocks" },
              { name:"Pineify UOA", url:"pineify.app/unusual-options-activity-scanner", desc:"Free unusual options activity scanner — Vol/OI ratio" },
              { name:"Market Chameleon", url:"marketchameleon.com", desc:"IV charts, options chains, unusual volume, earnings IV" },
              { name:"OptionStrat", url:"optionstrat.com", desc:"Strategy builder + live options flow feed (free delayed)" },
              { name:"Unusual Whales", url:"unusualwhales.com", desc:"Institutional flow scanner, dark pool, congressional trades" },
              { name:"Barchart", url:"barchart.com", desc:"GEX levels, options screener, IV percentile — free tier" },
            ].map(t=>(
              <div key={t.name} style={{ padding:"8px 10px", background:"rgba(255,255,255,0.015)", border:"1px solid rgba(255,255,255,0.04)", borderRadius:6 }}>
                <div style={{ fontSize:11, fontWeight:700, color:"#00ff88", marginBottom:2 }}>{t.name}</div>
                <div style={{ fontSize:9, color:"#2a4050", marginBottom:3 }}>{t.url}</div>
                <div style={{ fontSize:9, color:"#3d5a6e" }}>{t.desc}</div>
              </div>
            ))}
          </div>
        </div>

        <div style={{ textAlign:"center", marginTop:20, fontSize:9, color:"#0e1e26", letterSpacing:1 }}>
          AXIOM OPTIONS ENGINE v1.0 · BLACK-SCHOLES GREEKS · NOT FINANCIAL ADVICE · DYOR
        </div>
      </div>
